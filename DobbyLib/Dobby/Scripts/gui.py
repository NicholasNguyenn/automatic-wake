...
class GUI:
    def __init__(self, toggle_recording, get_robot_response, get_agent_state):
        self.master = tk.Tk()
    ...
    def log_console(self, text, system=False, end="\n"):
        ...
        if self.get_agent_state() != "CONVERSING":
            self.state_label.config(text=self.get_agent_state(), fg="blue")
        else:
            self.state_label.config(text=self.get_agent_state(), fg="black")
